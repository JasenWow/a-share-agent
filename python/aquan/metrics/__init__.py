...
def compile_query(
    ...
    for d in dimensions:
        if d not in allowed_dims:
            raise KeyError(f"Dimension '{d}' not allowed for metric '{metric}'. Allowed: {sorted(allowed_dims)}")
    for k in filters:
        if k not in allowed_dims:
            raise KeyError(f"Filter '{k}' not allowed for metric '{metric}'. Allowed: {sorted(allowed_dims)}")
