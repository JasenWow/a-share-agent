...
6. **Generate predictions.** For each stock: compute `predicted_pct` (next-day return %) and `confidence` (0-1). Apply strategy notes and error corrections. Apply exclusion rules (ST/*ST, suspended volume=0, IPO <30 days, limit-up/down). Max 20 stocks.
...
9. **Generate Markdown report.** Format: prediction table, accuracy tracking summary, error analysis summary, accuracy trend. Tag cold-start predictions.
10. **Save strategy notes.** Call `mcp__prediction_store__manage_strategy_notes` with `action="add"` to save key error patterns and strategy adjustments for next run.
