"""Performance metrics computation — accepts data, no DB access."""
...
def compute_performance(
    ...
    """Compute full performance metrics from NAV and trade data.

    Args:
        nav_rows: list of dicts with keys: trade_date, nav, cash, daily_return,
                  benchmark_return, excess_return, benchmark_value, benchmark_close
        trade_rows: list of dicts with keys: direction, amount, commission,
                    stamp_duty, slippage_cost
        initial_capital: starting capital

    Returns:
        Dict with 20+ performance metrics.
    """
    if not nav_rows:
    ...
    mean_excess_rf = sum(r - rf_daily for r in daily_returns) / n if n > 0 else 0.0
    ...
    total_buy_amount = sum(t.get("amount", 0) for t in trade_rows if t.get("direction") == "buy")
    ...
    total_commission = sum(t.get("commission", 0) for t in trade_rows)
    total_stamp_duty = sum(t.get("stamp_duty", 0) for t in trade_rows)
    total_slippage = sum(t.get("slippage_cost", 0) for t in trade_rows)
    ...
    return {
        ...
        "buy_trades": sum(1 for t in trade_rows if t.get("direction") == "buy"),
        "sell_trades": sum(1 for t in trade_rows if t.get("direction") == "sell"),
